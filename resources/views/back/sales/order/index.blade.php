                    ...
                    <div class="form-group row items-push mb-0">
                        ...
                        <div class="col-md-2 mb-0">
                            <select class="js-select2 form-control" id="filter-status-select" name="filter_status" style="width: 100%;" data-placeholder="{{ __('back/app.order.status_select') }}">
                                <option></option><!-- Required for data-placeholder attribute to work with Select2 plugin -->
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id }}" {{ $status->id == request()->input('status') ? 'selected' : '' }}>{{ $status->title->{current_locale()} ?? $status->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2 mb-0">
...
{{--                                </td>--}}
                                ...
                                <td class="text-center">{{ carbon($order->product->start_time)->format('d.m.Y h:i a') }}</td>
                                <td>{{ $order->product->translation()->title }}</td>
