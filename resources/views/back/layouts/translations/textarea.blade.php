        ...
        @foreach(ag_lang() as $lang)
            <div class="tab-pane fade show @if ($lang->code == current_locale()) active @endif" id="{{ $tab_title }}-{{ $lang->code }}-view" role="tabpanel" aria-labelledby="{{ $tab_title }}-{{ $lang->code }}-tab">
                <div class="form-group">
                    ...
                    <textarea class="js-maxlength form-control" id="{{ $tab_title }}-{{ $lang->code }}" name="{{ $input_name }}[{{ $lang->code }}]" rows="{{ $rows ?: 2 }}" @if($max_length)maxlength="{{ $max_length }}"@endif data-always-show="true" data-placement="top">
                        {!! isset($value->{$lang->code}) ? $value->{$lang->code} : '' !!}
                    </textarea>
                    @if ($max_length)
